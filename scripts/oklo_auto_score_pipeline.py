...
Examples:
  python oklo_auto_score_pipeline.py \
    --tickers OKLO \
    --date 2025-10-29 \
...
Multiple tickers (each会生成独立评分文件 + 合并总表):
  python oklo_auto_score_pipeline.py \
    --tickers OKLO LEU \
    --date 2025-10-29 \
...
def main():
    ap = argparse.ArgumentParser(description="End-to-end slicing + scoring pipeline")
...
if (total_min % args.interval) != 0:
    ...
    grabber = os.path.join(args.script-dir if hasattr(args, 'script-dir') else args.__dict__.get('script-dir', '.'), "us_stock_data_to_csv.py")
    scorer = os.path.join(args.script-dir if hasattr(args, 'script-dir') else args.__dict__.get('script-dir', '.'), "strength_score_cli_v3.py")
